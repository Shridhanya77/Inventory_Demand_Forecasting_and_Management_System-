...
router.use(authenticate, permit('center_manager'));

router.get('/dashboard', async (req, res, next) => {
  if (useMock) {
    const branchId = req.user.branch_id || 1;
    const stock = mockStore.state.branchStock
      .filter((item) => item.branch_id === Number(branchId))
      .map((item) => ({ ...item, name: mockStore.getComponentName(item.component_id) }));
    const lowStock = stock.filter((item) => Number(item.quantity) <= 5);
    return res.json({ stock, lowStock });
  }
  try {
...
router.get('/transactions', async (req, res, next) => {
  if (useMock) {
    return res.json(mockStore.getTransactionsByBranch(req.user.branch_id || 1));
  }
  try {
    const result = await db.query(
      `SELECT t.id, t.type, t.date, t.student_id, ti.component_id, c.name AS component_name, ti.quantity
       FROM transactions t
...
router.post('/students', async (req, res, next) => {
  if (useMock) {
    const { name } = req.body;
    const created = { id: mockStore.state.ids.students++, name, branch_id: req.user.branch_id || 1 };
    mockStore.state.students.push(created);
    mockStore.logAudit('register_student', `student:${created.id}`, req.user.id);
    return res.status(201).json(created);
  }
  try {
    const result = await db.query('INSERT INTO students (name, branch_id) VALUES ($1, $2) RETURNING *', [req.body.name, req.user.branch_id]);
...
router.post('/stock/add', async (req, res, next) => {
  if (useMock) {
    ...
    const row = mockStore.upsertBranchStock(req.user.branch_id || 1, component_id, Number(quantity));
    mockStore.logAudit('add_stock', `component:${component_id}`, req.user.id);
  ...
  try {
    const result = await db.query(
      `INSERT INTO branch_stock (branch_id, component_id, quantity)
       ...
       RETURNING *`,
      [req.user.branch_id, req.body.component_id, req.body.quantity]
    );
    res.status(201).json(result.rows[0]);
  } catch (error) {
    next(error);
...
router.post('/issue', async (req, res, next) => {
  if (useMock) {
    ...
    const item = items?.[0];
    const stock = mockStore.state.branchStock.find((s) => s.branch_id === (req.user.branch_id || 1) && s.component_id === Number(item?.component_id));
    if (!stock || stock.quantity < Number(item?.quantity || 0)) return res.status(400).json({ error: 'Insufficient stock' });
  ...
  try {
    // Manager shares the same DB behavior as staff: issue from their own branch.
    const branchId = req.user.branch_id;
...
router.post('/transfer-requests', async (req, res, next) => {
  if (useMock) {
    const created = {
      ...
      from_branch: Number(req.user.branch_id || 1),
      to_branch: Number(req.body.to_branch),
      component_id: Number(req.body.component_id),
      component_name: mockStore.getComponentName(req.body.component_id),
  ...
  try {
    const result = await db.query(
      ...
      [req.user.branch_id, req.body.to_branch, req.body.component_id, req.body.quantity, 'pending', req.user.id]
    );
    res.status(201).json(result.rows[0]);
  } catch (error) {
    next(error);
