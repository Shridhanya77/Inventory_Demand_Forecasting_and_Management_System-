...
const Issue = () => {
  ...
  const handleIssue = async (event) => {
    ...
    try {
      ...
      await api.post(`${getRoleApiBase()}/issue`, { student_id: studentId, items: [{ component_id: componentId, quantity: Number(quantity) }] });
      setMessage('Issue transaction completed successfully');
    } catch (err) {
      setMessage(err.response?.data?.error || 'Issue failed');
    }
  };

  return (
    <Box>
      <Typography variant="h4" mb={2}>Issue Component</Typography>
      <Paper sx={{ p: 3, maxWidth: 600, mb: 3 }}>
        ...
        <Box component="form" onSubmit={handleIssue} sx={{ display: 'grid', gap: 2 }}>
